...
v = [v j+i];
                                ...
                                [I] = Sub2ind([1:nx],v,1,nx,ny,nz);
                                rock.perm(I) = 10*10^(-per)*milli*darcy();
  
                    % Plot the permeability field
                    f(1)=figure(1);

                    plotCellData(G, rock.perm,'FaceColor','none');
...
%                     
                    ...
                    rSol = initState(G, [], 80*barsa, [0 1]);
                   % psolverop
                   [solver] = psolverop(lsolver,rSol, tol, maxIterations,cn,'wells',W);
                    ...
                    caxis([0 1]), view(0,90), axis equal off, title(['Water' heading]), colormap(jet)
                    subplot('position',[0.501, 0, 0.5, 0.9]), cla
                    plotCellData(G, rSol.s(:,2));
                    caxis([0 1]), view(0,90), axis equal off, title(['Oil' heading]), colormap(jet)
                    ...
                    t      = 0;
                    plotNo = 1;
                    H1 = 'Water Sat - '; H2 = 'Oil Sat - '; H3 = 'Water P - ';
                    e = []; p_org = []; p_pc = [];
                    figure;
                    %dT=20;
                    ts=0;
